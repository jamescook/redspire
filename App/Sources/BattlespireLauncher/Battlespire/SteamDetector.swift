...
/// Finds an existing Steam install of Battlespire (AppID 1812420) without
/// needing steamcmd or the user typing anything. Parses just enough of
/// Valve's VDF (KeyValues) text format with regexes -- proportionate for
/// pulling two field values out of two small config files, rather than
/// pulling in a full VDF parser for it.
enum SteamDetector {
    ...
    static func findGameDirectory(
        ...
        for library in candidateLibraryPaths(homeDirectory: homeDirectory, fileProvider: fileProvider) {
            ...
            if fileProvider.fileExists(atPath: gameExe) {
                return gameDir
            }
        }
        return nil
